trigger ChatterFeedProjectAfter on FeedItem (after insert) {
	...
	if (settings.Review_Request_On_Chatter__c == true) {
		...
		try {
			if(UserInfo.getName() == 'NA System User' && ExecuteTrigger__c.getAll().get('ChatterFeedProjectAfter').NotRunTrigger__c) {
	    		return;
			}
		} catch (Exception e) {
